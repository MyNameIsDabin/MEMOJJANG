...
**이번 판에서 바뀐 것을 `docs/release/<태그>.md` 에 먼저 적습니다.** 태그를 밀면 워크플로가
이 파일을 릴리스 노트 맨 위에 붙입니다 — 없으면 받는 곳 안내만 나가고, 그러면 사람들은
무엇이 좋아졌는지 모른 채 판올림을 받습니다. 태그와 **같은 커밋**에 들어가야 합니다.
...
- 화면에 나오는 글은 `src/i18n/` 에서만 옵니다. 컴포넌트 안에서는 `useT()`, 바깥에서는 `t()`.
  `ko.ts` 가 원본이라 열쇠를 거기 먼저 넣어야 `MessageKey` 가 생기고, 나머지 사전은 안 채워도
  타입이 통과합니다 — 그러니 **세 벌을 같이 채웠는지는 눈으로 확인해야 합니다.**
- README 는 세 벌(`README.md` · `README.en.md` · `README.ja.md`)입니다. **한 벌만 고치지 않습니다.**
  손이 덜 가는 쪽이 조용히 낡고, 낡은 문서는 없는 것만 못합니다. 기능을 바꿨으면 세 곳을 함께 고칩니다.
  스크린샷은 셋이 같은 파일(`docs/*.png`)을 씁니다 — 판마다 찍으면 그때부터 어긋나기 시작합니다.
