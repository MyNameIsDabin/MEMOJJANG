...
끝없는 캔버스에 원하는 위치로 툭툭 던져놓고, 휠로 밀고 `Ctrl`+휠로 확대·축소하면서 씁니다.

> **AI 로 만든 프로젝트입니다.**
> 코드와 아이콘, 이 문서까지 대부분을 Anthropic 의 Claude 와 함께 썼습니다.
> 무엇을 만들지와 어떻게 동작해야 하는지는 사람이 정하고, 구현과 다듬기를 AI 가 맡는 식으로 진행했습니다.

![메모짱 — 할 일, 마크다운 메모, 바로가기, 그림 노트가 한 판 위에 놓여 있는 모습](docs/board.png)
...
- **무한 캔버스** — 휠로 밀고 `Ctrl`+휠로 확대·축소(커서 기준), 빈 곳을 끌어 이동.
  100%·200% 근처에서는 살짝 붙잡아 픽셀이 또렷하게 유지됩니다.
  확대·축소에 `Ctrl` 을 요구하는 이유는, 긴 메모를 끝까지 굴린 순간 배율이 튀어 버리는 일을 막기 위해서입니다.
- **노트 네 종류**
  ...
  - **그림** — 붙여넣기(`Ctrl+V`), 브라우저에서 그림을 끌어다 놓기, 또는 우클릭 → **그림 불러오기**로 생깁니다.
    크기를 바꿔도 원본 비율을 지켜서 잘리거나 남는 자리가 없습니다.
...
- **찾기** — `Ctrl+F` 는 제목과 본문을 함께 뒤집니다. 상태 줄 왼쪽의 `🔍` 은 노트 목록을 펼쳐
  이름으로 골라 가는 쪽이고, 검색칸은 목록 아래에 붙어 있습니다. 어느 쪽이든 노트를 고르면 화면 한가운데로 오고,
  **펼쳐 놓고 보던 중이었다면 그 자리에 고른 노트가 대신 올라옵니다.**
- **판올림** — 설정 맨 위에서 새 판이 있는지 알려 주고, 누르면 받아서 깔고 다시 띄웁니다.
  받은 파일은 서명을 확인한 뒤에만 설치합니다.
- **정리와 스냅** — 겹쳐서 엉킨 보드를 격자에 맞춰 줄줄이 늘어놓거나, 전부 보이도록 확대율을 맞춥니다. 스냅을 켜면 노트를 놓는 순간 격자에 딱 붙습니다.
...
| `F2` | 고른 노트의 이름 바꾸기 |
| `Esc` | 화면 가득 펼친 노트 접기 · 열린 패널 닫기 · 선택 해제 |
| 휠 | 위아래로 밀기 — 내용이 넘쳐 스크롤이 생긴 노트 위에서는 그 안을 굴립니다 |
| `Shift`+휠 | 좌우로 밀기 (스크롤 되는 노트 위에서는 그 안을 좌우로) |
