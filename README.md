...
- **꾸미기** — 위쪽 도구 줄의 스티커 단추를 누르면 꾸미기 모드가 됩니다. 이 동안 노트는 잠기고
  캔버스 이동과 스티커만 다룹니다. 빈 곳을 우클릭해 스티커를 고르면 그 자리에 붙고, 귀퉁이 손잡이를
  끌어 **돌리고 키웁니다**(`Shift` 면 크기만). 스티커는 기본적으로 노트 뒤·캔버스 위에 놓입니다.
  왼쪽 아래 사슬 손잡이를 노트로 끌면 그 노트에 **붙어서 함께 움직이고**, 연결선에 마우스를 얹으면
  가위가 되어 누르는 순간 끊어집니다. 앞뒤 순서는 옆의 단추로 바꿉니다. `Enter` 로 배치를 마칩니다.
