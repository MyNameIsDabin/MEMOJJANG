...
마크다운은 **그려 둔 채로 고칩니다.** `#` 나 `**` 같은 표시는 감춰지고 제목은 실제로 커지는데,
캐럿을 그 자리에 놓으면 표시가 다시 드러납니다 — 고치려면 보여야 하니까요. 따로 편집 모드가 없습니다.
코드 블록은 울타리(` ``` `)째 접혀 한 덩어리로 보이고, 안으로 들어가면 울타리가 되돌아옵니다.

![JSON · HTML · 마크다운 메모가 각각 제 모습으로 그려진 화면](docs/views.png)

### 마감은 알람 타이머처럼

할 일 항목에 마우스를 얹으면 시계 단추가 나옵니다. **시·분·초를 넣고 시작**을 누르면 그때부터 셉니다.
단위를 넘겨 적어도 됩니다 — `90` 분은 한 시간 반으로 그냥 더해집니다. 날짜까지 정해야 하는 마감은
아래 칸에 직접 적습니다. 어느 쪽이든 목록 오른쪽에 `4일 남음` `20분 남음` 처럼 남은 시간과
진행도 막대가 붙습니다. 왼쪽 손잡이를 끌어 순서도 바꿉니다.

![할 일 항목의 마감 설정 팝업 — 시·분·초를 넣는 타이머 칸과 날짜 칸](docs/due.png)
...
- **캔버스 여러 개** — 캔버스 하나가 파일 하나입니다. 원하는 자리에 만들어 두고 위쪽 탭으로 오갑니다. 업무용·개인용을 나누거나, 프로젝트마다 하나씩 두고 쓰면 됩니다.
  탭 줄 오른쪽 **`+`** 를 누르면 새로 만들지, 폴더에서 찾을지, **최근에 열던 것**에서 고를지 물어봅니다 — 탭을 닫은 캔버스도 여기서 되찾습니다.
...
- **메모지 통째로 복사** — 노트를 고르고 `Ctrl+C`, 붙이고 싶은 자리에서 `Ctrl+V`.
  안쪽 글자가 아니라 **노트 한 장이 그대로** 복제되고, 붙어 있던 스티커도 함께 갑니다.
  다른 캔버스 탭에 붙여도 되며, 그림은 그 캔버스의 `.assets` 로 알아서 옮겨집니다.
  그림 노트 한 장을 골랐을 때는 **그림 자체도 함께** 클립보드에 올라가서, 그림판이나 메신저에
  바로 붙습니다 — 연필로 덧그린 것이 있으면 그려진 그대로 나갑니다.
...
- **말** — 한국어 · English · 日本語. 설정 맨 위에서 바꿉니다. 글꼴이 갈무리라 지금은 갈무리가 담고 있는
  글자에 맞는 말만 올려 두었고, 사전 파일 하나를 더하면 언어를 늘릴 수 있습니다 ([`src/i18n/`](src/i18n/)).
- **전역 단축키** — 기본 `Ctrl+Shift+Space`. 어느 프로그램에서든 불러내고 한 번 더 누르면 숨깁니다. 설정에서 원하는 조합을 직접 눌러 바꿀 수 있습니다.
- **어디서든 화면 캡처** — 기본 `Shift+PrintScreen`. 다른 프로그램을 쓰던 중에 눌러도 **그 화면 그대로** 찍힙니다 — 메모짱 창은 찍은 뒤에 올라옵니다. 이것도 설정에서 바꿉니다.
  설정에서 **찍기 전에 메모짱 창 감추기**를 켜면 메모짱이 잠깐 비켜 줍니다. 기본은 꺼짐 — 보드까지 함께 담고 싶을 때가 있어서입니다.
...
- **컴퓨터 켤 때 함께 시작** — 창을 띄우지 않고 트레이에만 올라옵니다. **기본값은 꺼짐**이고, 설정에서 켭니다.
...
| `Shift+PrintScreen` | **어디서든** 화면 캡처 시작 (바꿀 수 있음) |
...
src/
  ...
  notes/                NoteShell(공통 껍데기) + 종류별 본문 4종
    LiveMarkdown.tsx    그려 둔 채로 고치는 칸 (CodeMirror 를 얹는 자리)
    livePreview.ts      마크업을 감추고 제목을 키우는 꾸밈 — 캐럿이 놓인 자리만 원문으로
    detect.ts           붙여넣은 글로 보기 방식 추론 (기본 규칙 + 사용자 규칙)
  actions/
    capture.ts          캡처 시작 — 메뉴에서도, 전역 단축키에서도 같은 길
  ...
  ui/
    ...
    confirm.ts          되돌릴 수 없는 일을 하기 전에 한 번 더 묻는 공용 팝업
    ThemeEditor.tsx     색을 하나씩 고치고 되돌리는 편집기
    MemoRules.tsx       보기 방식 판단 규칙 관리
src-tauri/src/
  lib.rs                플러그인 등록, 닫기 동작, 명령
  files.rs              임의 경로 파일 읽기·쓰기·폴더 훑기 (fs 플러그인 범위 밖)
  net.rs                사이트 아이콘 내려받기
  capture.rs            바탕화면을 얼려 찍고 고른 자리를 오려 주기
  tray.rs               트레이 아이콘과 메뉴
  autostart.rs          컴퓨터 켤 때 함께 시작 (레지스트리 Run 키)
...
그림 노트를 지워도 그림 파일은 **그 자리에서 지우지 않습니다.** `Ctrl+Z` 로 노트가 돌아왔을 때
그림도 함께 돌아와야 하기 때문입니다. 아무도 가리키지 않게 된 그림은 그 캔버스를 **다음에 열 때** 한꺼번에 치워집니다.
