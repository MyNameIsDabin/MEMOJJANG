...
마크다운은 **그려 둔 채로 고칩니다.** 캐럿이 놓인 그 한 줄만 원문이 드러나고, 줄을 떠나면 곧바로 다시 그려집니다.
목록 안에서 Enter 를 누르면 표식이 이어 붙고, 빈 항목에서 한 번 더 누르면 목록에서 빠져나옵니다.
...
src/
  ...
  notes/                NoteShell(공통 껍데기) + 종류별 본문 4종
    Markdown.tsx        작은 마크다운 렌더러 — 그린 것마다 원본 줄 번호를 달아 둔다
    LiveMarkdown.tsx    그려 둔 채로 고치기 — 캐럿이 놓인 한 줄만 원문으로
